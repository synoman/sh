...
linux_tools() {
  ...
  while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}21.  ${gl_bai}黑客帝国屏保                      ${gl_kjlan}22.  ${gl_bai}走る電車のスクリーンセーバー"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}13.  ${gl_bai}ユーザー管理${gl_kjlan}14.  ${gl_bai}用户/密码生成器"
	...
	while true; do
		...
		case "$Limiting" in
			...
			4)  # 重命名目录
				...
				mv "$current_name" "$new_name" && echo "目录已重命名" || echo "名前の変更に失敗しました"
				send_stats "重命名目录"
				;;
			5)  # 删除目录
