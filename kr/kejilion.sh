...
delete_forwarding_service() {
	...
	echo "服务 $service_namefrpc.toml에서 성공적으로 제거되었습니다."
...
rsync_manager() {
	...
	while true; do
		clear
		echo "Rsync 远程同步工具"
		echo "원격 디렉터리 간 동기화는 효율적이고 안정적인 증분 동기화를 지원합니다."
		echo "---------------------------------"
		list_tasks
		echo
		view_tasks
...
docker_tato() {
	...
	if command -v docker &> /dev/null; then
		echo -e "${gl_kjlan}------------------------"
		echo -e "${gl_lv}환경이 설치되었습니다.${gl_bai}  容器: ${gl_lv}$container_count${gl_bai}  镜像: ${gl_lv}$image_count${gl_bai}  网络: ${gl_lv}$network_count${gl_bai}  卷: ${gl_lv}$volume_count${gl_bai}"
	fi
}
...
if command -v docker &>/dev/null; then
	if docker ps --filter "name=nginx" --filter "status=running" | grep -q nginx; then
		echo -e "${gl_huang}------------------------"
		echo -e "${gl_lv}환경이 설치되었습니다${gl_bai}  站点: $output  数据库: $db_output"
	fi
fi
